...
export const assert = (condition: boolean, testName: string): void => {
  ...
  /**
   * Check if a value is null or undefined and log the result
   */
  export const assertNotNullOrUndefined = <T>(actual: T, testName: string): void => {
    if (actual === null || actual === undefined) {
      throw new Error(
        "%cThe following test failed: " + testName + ". Expected a non-null and non-undefined value but got: " + actual
      );
    } else {
      console.log("%cThe following test passed: " + testName, "color: green");
    }
