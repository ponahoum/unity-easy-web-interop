...
public class BindingsSecondaryTest : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        ServiceRegisterer.RegisterMethod("asimplereturnstringmethod", MyMethodReturningString);
        ServiceRegisterer.RegisterMethod("asimplereturndoublemethod", MyMethodReturningDouble);
        ServiceRegisterer.RegisterMethod("asimplereturnintmethod", MyMethodReturningInt);
        ServiceRegisterer.RegisterMethod("ComputeStuff", ComputeStuff);
        ServiceRegisterer.RegisterMethod(nameof(MeMethodReturningDoubleArray), MeMethodReturningDoubleArray);
        ServiceRegisterer.RegisterMethod<Task<string>, string>("GetStringIfTaskComplete", GetStringIfTaskComplete);
        ServiceRegisterer.RegisterMethod<Action<string>, Task<string>>("ComputeStuffAndRaiseCallback", ComputeStuffAndRaiseCallback);
        ServiceRegisterer.RegisterMethod<string, string, string>("concatenatestrings", ConcatenateStrings);
        ServiceRegisterer.RegisterMethod<double, double>("addonetodouble", AddOneToDouble);
